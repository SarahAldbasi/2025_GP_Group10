...
void loop() {
  ...
  if (millis() - lastMeanTime >= 1000) {
    ...
    float diff = meanE2 - meanE1;

    Serial.println("\n------------------------------");
    Serial.print(" Anchor E1 Mean: "); Serial.println(meanE1, 2);
    Serial.print(" Anchor E2 Mean: "); Serial.println(meanE2, 2);
    Serial.print(" Difference (E2 - E1): "); Serial.println(diff, 2);
    Serial.println("------------------------------");

    if (diff >= 1.2) {
      Serial.println("OUTSIDE GOAL AREA");
    } else if (diff > 0.9 && diff < 1.2) {
      Serial.println(" ON THE GOAL LINE");
    } else {
      Serial.println(" GOAL DETECTED!");
      client.publish(MQTT_TOPIC_GOAL, "GOAL", false);
      Serial.println(" MQTT → GOAL sent");
    }
  }
  // =============================================================
